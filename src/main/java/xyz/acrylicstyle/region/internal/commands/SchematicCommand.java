...
public class SchematicCommand extends PlayerCommandExecutor {
    ...
    public void onCommand(@NotNull Player player, String[] args) {
        ...
        if (args[0].equals("load")) {
            ...
            if (!file.exists()) {
                player.sendMessage(ChatColor.RED + "Schematics folder does not exist.");
                return;
            }
            for (File f : file.listFiles(f -> f.isFile() && f.getName().endsWith(".schem") || f.getName().endsWith(".schematic"))) {
                ...
                load.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/" + RegionEditPlugin.COMMAND_PREFIX + "schem load " + f.getName().replaceFirst("(.*)\\.(schem|schematic)", "$1")));
                TextComponent name = new TextComponent(ChatColor.LIGHT_PURPLE + f.getName());
                text.addExtra(load);
                text.addExtra(name);
